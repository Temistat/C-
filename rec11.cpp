/*
  ...
  recitation.
  Temiloluwa Omomuwasan section B too2007
*/
...
const Node* isSublist(const Node* pattern, const Node* target) {
    const Node* backTarget = target;
    const Node* backPattern = pattern;
    while (pattern != nullptr && target != nullptr) {
        if (target->data == pattern->data) {
            target = target->next;
            pattern = pattern->next;
        } else {
            target = backTarget->next;
...
const Node* sharedListBruteForce(const Node* listA, const Node* listB) {
    for (const Node* nodeA = listA; nodeA != nullptr; nodeA = nodeA->next) {
        for (const Node* nodeB = listB; nodeB != nullptr; nodeB = nodeB->next) {
            if (nodeA == nodeB) {
                return nodeB;
            }
        }
